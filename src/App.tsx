...
import useFindUser from './api/FindUserApi'

function App() {
  const nav = useNavigate()
  const {findUserApi} = useFindUser()
  ...
  return (
    ...
    <div className="flex flex-col justify-center h-screen items-center text-center gap-12">
      ...
      <form action="" className='flex flex-col w-60 gap-3.5' onSubmit={handleSubmit(findUserApi)}>
        <div className="relative">
          ...
          {errors.findUserByName && <p className='text-sm text-error'>Enter username</p>}
        </div>
