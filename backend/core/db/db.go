...
func (db *DB) CreateUser(user *models.User, username, password string) error {
	...
	if err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, map[string]string{"createUser": "failed to create uuid for user"})
	}
	...
	if err := createResult.Error; err != nil {
		...
		if strings.HasSuffix(err.Error(), "(2067)") {
			return echo.NewHTTPError(http.StatusConflict, map[string]string{"username": "user with such username already exists"})
		}
		return echo.NewHTTPError(http.StatusInternalServerError, map[string]string{"createUser": "failed to create user: " + err.Error()})
	}
	return nil
}

// получение юзера по его ID
func (db *DB) GetUserByID(user *models.User, userID uuid.UUID) error {
	selectResult := db.dbConnect.First(user, userID)
	if err := selectResult.Error; err != nil {
		// если ошибка в ненахождении записи
		if err.Error() == notFoundError {
			return echo.NewHTTPError(http.StatusNotFound, map[string]string{"getUser": "user with such id was not found"})
		}
		return echo.NewHTTPError(http.StatusInternalServerError, map[string]string{"getUser": "failed to get user by id: " + err.Error()})
	}
	return nil
}
...
func (db *DB) GetUserByUsername(user *models.User, username string) error {
	...
	if err := selectResult.Error; err != nil {
		// если ошибка в ненахождении записи
		if err.Error() == notFoundError {
			return echo.NewHTTPError(http.StatusNotFound, map[string]string{"getUser": "user with such username was not found"})
		}
		return echo.NewHTTPError(http.StatusInternalServerError, map[string]string{"getUser": "failed to get user by username: " + err.Error()})
	}
	return nil
}

// получение чата (с подгрузкой его участников и сообщений) по его ID
func (db *DB) GetFullChatByID(chat *models.Chat, chatID uuid.UUID) error {
	...
	).Preload("Messages.Sender").First(chat, chatID)

	if err := selectResult.Error; err != nil {
		// если ошибка в ненахождении записи
		if err.Error() == notFoundError {
			return echo.NewHTTPError(http.StatusNotFound, map[string]string{"getChat": "chat with such id was not found"})
		}
		return echo.NewHTTPError(http.StatusInternalServerError, map[string]string{"getChat": "failed to get chat by id: " + err.Error()})
	}
	return nil
}

// получение чата (с подгрузкой его участников) по его ID
func (db *DB) GetChatParticipantsByID(chat *models.Chat, chatID uuid.UUID) error {
	selectResult := db.dbConnect.Preload("Users").First(chat, chatID)
	if err := selectResult.Error; err != nil {
		// если ошибка в ненахождении записи
		if err.Error() == notFoundError {
			return echo.NewHTTPError(http.StatusNotFound, map[string]string{"getChat": "chat with such id was not found"})
...
func (db *DB) createChat(chat *models.Chat, firstUserFromDB, secondUserFromDB models.User) error {
	...
	newUUID, err := uuid.NewRandom()
	if err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, map[string]string{"createChat": "failed to create uuid for chat"})
	}
	...
	if err := createResult.Error; err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, map[string]string{"createChat": "failed to create chat"})
	}
	return nil
}
...
func (db *DB) CreateMessage(message *models.Message, chatID, senderID uuid.UUID, content string) error {
	...
	newUUID, err := uuid.NewRandom()
	if err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, map[string]string{"createMessage": "failed to create uuid for message"})
	}
	...
	if err := createResult.Error; err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, map[string]string{"createMessage": "failed to create message: " + err.Error()})
	}
